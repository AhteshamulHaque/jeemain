...
@login_required
def admin_home():
	conn = connect("localhost","root","","jee")
	cursor = conn.cursor()

	if request.args.get('select_type') == 'reg_incom':
		cursor.execute("SELECT id,name,physics,chemistry,maths,photo,signature,marksheet,upload_verified,info_verified,step_2,step_3 FROM student WHERE step_2 = 0 OR step_3 = 0")

	elif request.args.get('select_type') == 'verified':
		cursor.execute("SELECT id,name,physics,chemistry,maths,photo,signature,marksheet,upload_verified,info_verified,step_2,step_3 FROM student WHERE upload_verified IS NOT NULL AND info_verified IS NOT NULL")

	elif request.args.get('select_type') == 'not_verified':
		cursor.execute("SELECT id,name,physics,chemistry,maths,photo,signature,marksheet,upload_verified,info_verified,step_2,step_3 FROM student WHERE upload_verified IS NULL OR info_verified IS NULL")
	else:
		cursor.execute("SELECT id,name,physics,chemistry,maths,photo,signature,marksheet,upload_verified,info_verified,step_2,step_3 FROM student")
	...
	return render_template('admin/home.html',title="Admin",students=result,notifications=notifications,no_of_notifs=notifs[0][0])
...
def change_data():
	...
	if request.method == "POST":
		cursor.execute("UPDATE student SET physics=%.1f,chemistry=%.1f,maths=%.1f WHERE id=%d"%(float(request.form['physics']),float(request.form['chemistry']),float(request.form['maths']),int(request.form['id'])))
		conn.commit()
		flash("Marks of student changed successfully")

	cursor.execute("SELECT id,name,email,physics,chemistry,maths FROM student WHERE id=%d"%int(request.args.get('id')))
	result = cursor.fetchall()
	conn.close()
...
def update_ranks():
	...
	for rank,values in enumerate(result):
		cursor.execute("UPDATE student SET physics=%d,chemistry=%d,maths=%d,AIR=%d WHERE id=%d"%(values[1],
			values[2],values[3],rank+1,values[0]))
	...
	if request.method == "POST":
		...
		cursor.execute('''UPDATE student SET address='{}',applying_for='{}',category='{}',
			...
			upload_verified={},info_verified={} WHERE id={}'''.format(r['address'].upper(),
				r['applying_for'].upper(),r['category'].upper(),r['date_of_birth'],r['email'],r['father_name'].upper(),r['mother_name'].upper(),
				r['gender'].upper(),r['mode_of_exam'].upper(),r['name'].upper(),r['paper_medium'].upper(),r['phone'],r['pwd'],r['state_of_eligibility'].upper(),1,1,int(request.args.get('id'))))
		conn.commit()
