...
def register_student():
	...
	if "http://localhost/student/register/info" in request.referrer:
...
'{mode_of_exam}','{paper_medium}','{address}','{email}','{phone}','{passwd}',1) '''.format(newID=newID,**session,passwd=request.form['passwd']))

		cursor.execute('''INSERT INTO credential(id,name,password) VALUES(%d,'%s','%s')'''%(newID,session['username'],request.form['passwd']))
		conn.commit()
		conn.close()
	...
	elif "http://localhost/student/register/upload" in request.referrer:
	...
	elif "http://localhost/student/register/payment" in request.referrer:
...
def student_home():
	...
	cursor.execute("SELECT subject,message,resolved,id FROM claims WHERE id=%d AND resolved=1 AND seen_student=0"%(int(session['id'])))
	...
	return render_template('student/profile.html',title="Profile",notifications=notifications,no_of_notifs=notifs[0][0])
...
def notification():
	...
	return render_template('student/notification.html',title="Notification",all_notifications=all_notifications,no_of_notifs=0)
...
def claim():
	...
	cursor.execute("INSERT INTO claims VALUES(%d,'%s','%s','%s',0,0,0)"%(int(session['id']),session['username'],request.form['subject'],request.form['claim']))
	conn.commit()
	conn.close()
...
def reset_parse():
	...
	if not result:
		flash ("Invalid registration number. Please enter a valid registration id")
		return redirect(url_for('reset_option'))
...
def reset_from_otp(token):
	...
	if request.method == "POST":
		try:
			...
			if (request.args.get('method') == 'otp' and data['otp'] == request.form['otp']) or request.args.get('method') == 'email':
	...
	try:
		...
		if request.method == "POST":
			...
			if passwd != confirm_passwd:
				...
				cursor.execute("UPDATE student SET password='%s' WHERE id=%d"%(request.form['passwd'],int(ID)))
				cursor.execute("UPDATE credential SET password='%s' WHERE id=%d"%(request.form['passwd'],int(ID)))
				conn.commit()
				conn.close()
